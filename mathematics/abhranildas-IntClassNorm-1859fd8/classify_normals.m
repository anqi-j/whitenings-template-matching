function results=classify_normals(dist_1,dist_2,varargin)
	...
	colors=[0,0.447,0.741;0.85,0.325,0.0980;0.929,0.694,0.125;0.494,0.184,0.556;0.466,0.674,0.188;0.301,0.745,0.933;0.635,0.0780,0.184];
	...
	if strcmpi(parser.Results.input_type,'samp')
		...
		if parser.Results.samp_opt && strcmpi(dom_type,'quad') % if default boundary,
			try
				...
				x=fminsearch(@(x) -samp_value_flat(x,dist_1,dist_2,vals),[dom.q2(triu(true(size(dom.q2)))); dom.q1(:); dom.q0],optimset('Display','none','TolX',0,'TolFun',1/(size(dist_1,1)+size(dist_2,1))));
