function [lumpy_s,x,y] = evaluate_lumpy_s(imgs, replication)
...
for n=1:len
    ...
    k = polyfit(x(y~=0).^2,log(y(y~=0)),1);
    lumpy_ss(n) = real(sqrt(-k(1)))/pi;
end

lumpy_s = mean(lumpy_ss);
