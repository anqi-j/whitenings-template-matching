function img = create_power_noise_sample_field(row, col, crow, ccol, n, p, noise_mean, sta_dev)
% ---
% Operation: extract 1/(f^p) noise patches from a larger 1/(f^p) field with lower frequencies removed
% Input:
...
field_ratio = round(crow/row)/2; %background is twice the size of the target
scene = create_power_noise_field(crow, ccol, p, field_ratio, noise_mean, sta_dev); % rms contrast = sta_dev/noise_mean
